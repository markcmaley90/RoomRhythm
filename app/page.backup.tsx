...
function KeyboardHint() {
  return (
    <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center pointer-events-none select-none z-10" style={{gap: "280px"}}>
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center gap-8 p-8">
      ...
      <div className="flex flex-col sm:flex-row gap-6 w-full max-w-4xl">
        ...
        <button onClick={() => onSelect("testing")}
          className="flex-1 flex flex-col items-center gap-4 p-8 rounded-3xl bg-stone-800 hover:bg-stone-700 text-white transition-all shadow-xl hover:scale-105">
          <span className="text-6xl">📝</span>
          <span className="text-2xl font-bold">Testing</span>
          <span className="text-stone-300 text-sm text-center">Proctors & test administrators. Section timing, mandated warnings, extended-time support.</span>
        </button>
      </div>
  ...
  return (
    <div className={`min-h-screen ${config.bg} ${config.text} flex flex-col items-center justify-center transition-colors duration-700 p-6 relative ${emergencyActive ? "ring-8 ring-inset ring-red-500/60" : ""}`}>
      ...
      <div className="flex gap-4 mb-5 flex-wrap justify-center">
        ...
        <button onClick={() => activateMode("focus")} className="px-8 py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white text-lg font-semibold transition-all shadow-lg">⏱ Focus</button>
...
function CorporateApp({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className={`min-h-screen ${config.bg} ${config.text} flex flex-col items-center justify-center transition-colors duration-700 p-6 relative ${emergencyActive ? "ring-8 ring-inset ring-red-500/60" : ""}`}>
      ...
      {mode === "recharge" && (
        <div className="mb-8 px-6 py-4 bg-white/10 rounded-2xl text-center max-w-sm">
          <p className="text-xs uppercase tracking-widest opacity-60 mb-1">Recharge</p>
      ...
      <div className="flex gap-4 mb-5 flex-wrap justify-center">
        <button onClick={handleCalmClick} className="px-8 py-4 rounded-2xl bg-amber-500 hover:bg-amber-400 text-white text-lg font-semibold transition-all shadow-lg">🌿 Calm</button>
        <button onClick={() => activateCorporateMode("work")} className="px-8 py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white text-lg font-semibold transition-all shadow-lg">💼 Focus</button>
...
function TestingApp({ onBack }: { onBack: () => void }) {
  ...
  if (projector && current) {
    return <ProjectorView secondsLeft={secondsLeft} label={current.name} emoji="📝"
      nearEnd={nearEnd} totalDuration={totalDurationRef.current} onExit={() => setProjector(false)} />;
  }

  return (
    <div className="min-h-screen bg-stone-900 text-stone-100 flex flex-col items-center justify-center transition-colors duration-700 p-6 relative">
      ...
      {phase === "setup" && (
        <div className="flex flex-col items-center gap-6 w-full max-w-lg">
          ...
          <p className="opacity-60 text-sm -mt-4">Build your section schedule, then start the administration.</p>
          ...
          {sections.length > 0 && (
            <div className="w-full flex flex-col gap-2">
              {sections.map((s, i) => (
                <div key={s.id} className={`flex items-center gap-2 p-3 rounded-xl ${s.isBreak ? "bg-emerald-900/40" : "bg-white/10"}`}>
                  <span className="text-xs opacity-40 w-6">{i + 1}</span>
                  <input
                  ...
                  <input
                    ...
                    className="w-16 bg-white/10 rounded-lg px-2 py-1 text-sm text-center outline-none"
                    value={s.minutes}
          ...
          {/* Add buttons */}
          <div className="flex gap-3">
            <button onClick={() => addSection(false)} className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-sm font-medium transition-all">+ Section</button>
            <button onClick={() => addSection(true)} className="px-4 py-2 rounded-xl bg-emerald-900/60 hover:bg-emerald-800/60 text-sm font-medium transition-all">+ Break</button>
          </div>

          {/* Extended time toggle */}
          <button onClick={() => setExtTime((v) => !v)}
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${extTime ? "bg-white text-gray-900" : "bg-white/10 hover:bg-white/20"}`}>
          ...
          {sections.length > 0 && (
            <button onClick={startTest}
              className="px-10 py-4 rounded-2xl bg-stone-100 hover:bg-white text-stone-900 text-lg font-bold transition-all shadow-lg">
              Review & Start →
            </button>
          )}
        </div>
      ...
      {phase === "ready" && (
        <div className="flex flex-col items-center gap-6">
          <p className="text-sm uppercase tracking-widest opacity-50">Up first</p>
          <h1 className="text-5xl font-bold text-center">{sections[0].name}</h1>
          <p className="text-2xl opacity-70">{sections[0].minutes} minutes</p>
      ...
      {phase === "running" && current && (
        <div className="flex flex-col items-center gap-4">
          ...
          <h1 className="text-4xl font-bold text-center pb-1">{current.name}</h1>

          {/* 5-minute warning banner */}
          {show5MinWarning && (
            <div className="px-6 py-2 bg-red-900/60 border border-red-500/40 rounded-full text-red-200 font-semibold animate-pulse">
              ⚠️ {Math.ceil(secondsLeft / 60)} MINUTE{Math.ceil(secondsLeft / 60) !== 1 ? "S" : ""} REMAINING
            </div>
          )}

          {/* Clock(s) */}
          <div className={`flex ${extTime ? "gap-16" : ""} items-center`}>
            <div className="flex flex-col items-center">
              {extTime && <p className="text-xs uppercase tracking-widest opacity-50 mb-2">Standard</p>}
              <div className={`font-mono font-bold transition-all duration-300 ${extTime ? "text-7xl" : "text-9xl"} ${nearEnd ? "text-red-300 animate-pulse" : ""}`}>
                {formatTime(secondsLeft)}
              </div>
            </div>
            {extTime && (
              <div className="flex flex-col items-center">
                ...
                <div className={`font-mono font-bold text-7xl transition-all duration-300 ${extSecondsLeft <= 60 && extSecondsLeft > 0 ? "text-red-300 animate-pulse" : "opacity-80"}`}>
          ...
          <div className="flex gap-3 mt-4">
            <button onClick={() => setRunning((r) => !r)} className="px-5 py-2 rounded-xl bg-white/20 hover:bg-white/30 text-sm font-medium">
              {running ? "⏸ Pause" : "▶ Resume"}
            </button>
            <button onClick={resetAll} className="px-5 py-2 rounded-xl bg-white/20 hover:bg-white/30 text-sm font-medium">↺ End Test</button>
          </div>
        </div>
      )}
      ...
      {phase === "between" && (
        <div className="flex flex-col items-center gap-6">
          ...
          <p className="text-sm uppercase tracking-widest opacity-50">Up next</p>
          <p className="text-2xl font-semibold">{sections[currentIdx + 1].name} · {sections[currentIdx + 1].minutes} min</p>
      ...
      {phase === "done" && (
        <div className="flex flex-col items-center gap-6">
          ...
          <button onClick={resetAll}
            className="px-8 py-3 rounded-2xl bg-stone-100 hover:bg-white text-stone-900 font-bold transition-all shadow-lg">
            New Administration
          </button>
        </div>
      )}
